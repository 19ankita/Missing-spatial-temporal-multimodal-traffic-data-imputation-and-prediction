...
def load_inputs(basedir: Path, city, day: Optional[str] = None, df_filter: DF_FILTER = df_filter_weekdays_daytime_only) -> pd.DataFrame:
    ...
    fn = basedir / city / "input" / f"counters{infix}.parquet"
...
def load_cc_labels(
    basedir: Path, city, day=None, df_filter: DF_FILTER = df_filter_weekdays_daytime_only, with_edge_attributes: bool = False
) -> pd.DataFrame:
...
def load_eta_labels(basedir: Path, city, day=None, df_filter: DF_FILTER = df_filter_weekdays_daytime_only) -> pd.DataFrame:
    """Helper for laoding cc labels from basedir.
...
def cc_dates(basedir: Path, city) -> List[str]:
    """Load dates for which there is congestion data."""
    dates = [re.search("[0-9]{4}-[0-9]{2}-[0-9]{2}", str(fn)).group() for fn in (basedir / city / "input").rglob("counters_*.parquet")]
    #print(f"cc_dates: {dates}")  # Print the dates to debug
    return dates
