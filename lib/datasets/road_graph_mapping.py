...
class TorchRoadGraphMapping:
    def __init__(self, city: str, root: Path, df_filter, edge_attributes=None, skip_supersegments: bool = True, counters_only: bool = False):
        ...
        # Check if any of the dataframes are None and raise an error with specific message
        if df_edges is None:
            raise ValueError("df_edges is None.")
        if df_nodes is None:
            raise ValueError("df_nodes is None.")
        #if df_supersegments is None:
            #raise ValueError("df_supersegments is None.")
        
        print(f"df_edges: {df_edges.shape}, df_nodes: {df_nodes.shape}")
    ...
    def load_inputs_day_t(self, basedir: Path, city: str, day: str, t: int, idx: int) -> torch.Tensor:
        ...
        """
        df_x = load_inputs(basedir, city=city, day=day, df_filter=self.df_filter)

        df_x["node_id"] = df_x["node_id"].copy().astype("int64")
        df_x = df_x.explode("volumes_1h")
        assert len(df_x) % 4 == 0
        df_x = df_x.reset_index()
        df_x["slot"] = df_x.index % 4
        df_x["volumes_1h"] = df_x["volumes_1h"].copy().astype("float")
        
        #thesis
        # Extract 'volumes_1h' as a numpy array
        #volumes = np.array(df_x['volumes_1h'].tolist())

        # Create a mask: True where values are not NaN - valid data
        #mask = ~np.isnan(volumes)

        # You can store this mask as part of the data for later use
        #df_x['mask'] = mask

        #empty tensor is created of size (num_nodes, 4)
        ...
        data["node_index"] = [self.node_to_int_mapping[x] for x in data["node_id"]] #mapping the noid_id with indices properly
        ...
        # Create a mask tensor: True where values are not NaN
        mask = torch.full_like(x, False, dtype=torch.bool) #torch.full_like returns a tensor similar to x
        mask[data["node_index"].values, data["slot"].values] = ~torch.isnan(torch.tensor(data["volumes_1h"].values, dtype=torch.float))
    ...
    def load_cc_labels_day_t(self, basedir: Path, city: str, day: str, t: int, idx: int) -> torch.Tensor:
        ...
        """
        df_y = load_cc_labels(basedir, city=city, day=day, with_edge_attributes=True, df_filter=self.df_filter)
        if day == "test":
        ...
        if len(data) == 0:
            logging.warning(f"{city} {(idx, day, t)} no classified")
        return y

    def load_eta_labels_day_t(self, basedir: Path, city: str, day: str, t: int, idx: int) -> torch.Tensor:
        ...
        """
        df_y = load_eta_labels(basedir, city=city, day=day, df_filter=self.df_filter)
        if day == "test":
            data = df_y[(df_y["test_idx"] == idx)]
        else:
            data = df_y[(df_y["day"] == day) & (df_y["t"] == t)]
